...
### Foreground on intent — button text legibility (4)
...
These are the most important checks for interactive elements. A button with `background: primary, color: onPrimary` must pass here or the label is illegible. The library guarantees these pass by choosing `on*` colors from near-white or near-black — whichever has higher contrast with the base color.
